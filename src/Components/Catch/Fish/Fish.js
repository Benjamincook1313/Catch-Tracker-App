...
import { ToggleButton, ToggleButtonGroup, Button, FormControl, Dropdown, DropdownButton, InputGroup } from 'react-bootstrap';
...
function Fish(){
  ...
  const saltWater = ['Tarpon', 'Mahimahi'];
  ...
  return(
    <div className='Fish'>
      ...
      <ToggleButtonGroup className='ButtonGroup' type='radio' name='fishType' >
        ...
        <ToggleButton variant='outline-secondary' value="4" onClick={() => setType(saltWater)/
            setShowSpecies(true)/dispatch({type: 'SPECIES'})/
            dispatch({type: 'FISH_TYPE', payload: 'SaltWater'}) /setShowOther(false)}>
          <div className='type-btn'>Salt Water</div>
        </ToggleButton>
        <ToggleButton variant='outline-secondary' value="5" onClick={() => setType(other)/
            setShowSpecies(true)/dispatch({type: 'SPECIES'})/
